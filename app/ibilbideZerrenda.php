<?php 
  ...
  $stmt = $conn->prepare("SELECT * FROM ibilbideak WHERE NOT Email=?"); 
  $stmt->bind_param("s", $email);
  $stmt->execute();
  $ibilbideak= $stmt->get_result(); 
            ...
            <tbody>
              ...
              while($row=mysqli_fetch_array($ibilbideak)){
                ...
                "<tr>
                  ...
                  <td class=letra>{$row['Ibilbidearen_izena']}</td>
                  <td class=letra>{$row['Email']}</td>
                  <td class=letra>{$row['Zailtasuna']}</td>         
                  <td class=letra>{$row['Distantzia_m']}</td>
                  <td class=letra>{$row['Desnibela_m']}</td>
                 <td class=letra><button id='btnModal' type='button'><a href='{$row['Link']}'>Link-a</button></td>          
