...
function konprobaketa(evento) { 
	...
	if (/^[0-9]{8}[A-Z]$/.test(nan)){
		...
		return;	
     	...
     	if (!(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,15}/.test(pasahitza1))){
     		alert("Pasahitza ez da egokia. Gutxienez letra xehe bat, larri bat, zenbaki bat eta karaktere berezi bat izan behar du, espaziorik ezin du izan eta gutxienez 6 eta gehienez 15 karaktereko luzera izan behar du.");
     		return;
     	}
