...
if (!empty($_POST['csrf'])) {
	if (hash_equals($_SESSION['token'], $_POST['csrf'])) {
		...
		if($row['Email']!=null){ #erabiltzailea erregistratuta egotekotan 
			...
			error_log("127.0.0.1 user-identifier user_name [".date('r')."] 'Login ondo'".PHP_EOL,3,"./log/login.log");
			header("Location: http://localhost:81/erabiltzailearenMenua.php");     
		}
		else {
			date_default_timezone_set('Europe/Madrid');
			error_log("127.0.0.1 user-identifier user_name [".date('r')."] 'Login txarto'".PHP_EOL,3,"./log/login.log");
